...
Emotion DB 或产生 formal 写入。formal generation 只有一条 Fiber-owned one-shot
Timer 链；reload 会先停止旧链再启动新链。
...
工具幂等提交、TurnCommitted selection 时序、暂态 Timer 重臂和 reload 单 Timer。
